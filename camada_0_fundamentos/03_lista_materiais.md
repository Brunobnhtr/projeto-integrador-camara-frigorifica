...
| **KA1 a KA4 — Relé 8 pinos 24 Vcc + base DIN** | **5** ⭐ | ⭐ **UM MODELO SÓ PARA O PAINEL INTEIRO** — 4 em uso + 1 reserva, e a reserva cobre qualquer um dos quatro. Relé eletromecânico **8 pinos, bobina 24 Vcc, 2 contatos reversíveis (2 NA + 2 NF) de 10 A cada**, com **base para trilho DIN inclusa**. Buscar `relé 8 pinos 24v 10a base din` · `JQX-13F 24v` · `LY2N 24vdc`. ~R$ 39 o conjunto. **KA1** usa os 2 contatos (selo + saída para o KA2); **KA2** usa 1 contato para os 6,0 A e sobra o outro; **KA3** usa 1 contato NA (37 mA) e **KA4** usa 1 contato **NF** (0,36 A) — nos dois sobra um reversível inteiro. 🔄 **Eram 3; passou a 5 quando o KA3 e o KA4 deixaram de ser módulos de 5 V ([Doc 31 §31.13](../camada_3_eletrica/31_comando_e_protecoes.md))** | |
...
| 🗑️ ~~Módulo relé 1 canal 5 V, optoacoplado~~ | ~~4~~ | **SAIU DO PROJETO.** O KA3 e o KA4 viraram relés de 8 pinos, iguais ao KA1/KA2 — já contabilizados na linha acima. Motivo: **um modelo só no painel**, 2 contatos reversíveis em vez de 1 (o `NF` do KA4 sai de graça), **130 mA a menos no BD-5V** e montagem direta no trilho. [Doc 31 §31.13](../camada_3_eletrica/31_comando_e_protecoes.md) | |
| 🗑️ ~~Caixa modular DIN 4 módulos (70 mm)~~ | ~~1~~ | **SAIU JUNTO.** Relé de painel monta no trilho, não em caixa de plástico. Os 70 mm voltaram para o trilho 2 | |
| ⭐ **MOSFET 2N7000 (TO-92)** | 4 | **`Q3` e `Q4`** — 2 em uso + 2 reserva. O driver entre o pino do Mega (5 V) e a bobina de 24 V: 37 mA contra os 200 mA que ele aguenta, Vds 60 V. **Não vai em placa** — vai num termorretrátil preso na base do próprio relé. ~R$ 0,60 cada | |
| ⭐ **Resistor 10 kΩ · ¼ W** | 4 | **`R10` e `R11`** — pull-down do **gate** de cada MOSFET para o 0 V, dentro do mesmo termorretrátil. 2 em uso + 2 reserva. ⚠️ **É o que torna o fail-safe medível:** Arduino ausente ou fio rompido → gate em 0 V → bobina solta. No KA3 isso corta a potência; no KA4, **liga a ventoinha** | |
| ⭐ **Diodo 1N4007** | **6** ⭐ | **`D1` (bobina do KA2), `D2` (ventoinhas do radiador), `D3` (bobina do KA3) e `D4` (bobina do KA4)** — 4 em uso + 2 reserva. 🔄 **Eram "não compre"; passaram a 6** porque as bobinas novas do KA3 e do KA4 precisam de roda-livre próprio, senão o MOSFET morre no primeiro desligamento. 🔥 **O papel de cada um:** o `D3` e o `D4` protegem os MOSFET Q3/Q4; o `D1` protege o **contato do KA3** — contato seco abrindo uma bobina forma arco, e um contato que pita acaba soldando — o veto do firmware perdido em silêncio. 🔎 **O `D1` pode ser dispensável:** teste o KA2 em *teste de diodo* entre `A1` e `A2`; se conduzir num sentido só, ele já tem diodo interno. ⚠️ **O `D2` é obrigatório** — o motor é indutivo e nada o grampeia. Catodo no **+12 V** | |
...
>
> ⭐ **O KA3, o KA4 e os conjuntos Q3/Q4 NÃO vão na PI-1.** Os dois relés moram **no próprio trilho 2**, lado a lado, e cada MOSFET vai num termorretrátil preso à base do seu relé. A primeira versão punha o MOSFET na PI-1 e o `npm run valida` reprovou: o circuito da bobina teria de atravessar o painel duas vezes, uma delas pela canaleta de sinal. **Componente de ancoragem é de proximidade** — e é por isso que o R10 fica colado ao gate, e não na placa.
>
